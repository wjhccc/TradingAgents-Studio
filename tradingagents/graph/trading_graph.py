...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        Args:
            ...
            on_node_complete: Optional callable invoked after each graph node.
                              May accept either ``(state_keys)`` (legacy, single
                              positional) or ``(state_keys, chunk)`` — the latter
                              receives the full merged ``stream_mode="values"``
                              state at the moment the node completed, used by the
                              web layer to surface per-turn debate content.
        """
        self.debug = debug
    ...
    def _create_tool_nodes(self) -> Dict[str, ToolNode]:
        ...
        return {
            ...
            # CN Social, Event, Capital Flow, and Macro analysts use the
            # pre-fetch pattern (no LLM tool calling). Empty tool nodes
            # satisfy GraphSetup's requirement that every analyst key in
            # the plan has a matching entry in this map.
            "cn_social": ToolNode([]),
            "event": ToolNode([]),
        ...
        if self.debug:
            ...
            for chunk in self.graph.stream(init_agent_state, **args):
                ...
                if self.on_node_complete:
                    self._invoke_node_callback(chunk)
    ...
    def _invoke_node_callback(self, chunk):
        """Call ``on_node_complete`` with whichever signature the caller declares.

        The web layer wants the full chunk to extract per-turn debate content;
        legacy callers only want the list of updated state keys. Inspect the
        callable's parameter count once and pass the right shape.
        """
        try:
            ...
            try:
                ...
                params = inspect.signature(self.on_node_complete).parameters
                if len(params) >= 2:
                    self.on_node_complete(keys, chunk)
                    return
            except (TypeError, ValueError):
                pass
            self.on_node_complete(keys)
        except Exception:
            pass
    ...
    def _log_state(self, trade_date, final_state):
        ...
        self.log_states_dict[str(trade_date)] = {
            ...
            "event_impact_report": final_state.get("event_impact_report", ""),
